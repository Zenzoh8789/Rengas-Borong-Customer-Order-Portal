...
export type CustomerRegistration = Omit<CustomerProfile, "id"> & {
  password: string;
};
...
export const api = {
  categories: () => request<StoreCategory[]>("/store/categories"),
  ...
  cachedProducts: () =>
    productSnapshot?.key ===
    (localStorage.getItem("rengas-token") || "")
  ...
  products: (): Promise<Product[]> => {
    ...
    if (
      !productsRequest ||
      key !== productsKey ||
      Date.now() - productsLoadedAt > 60_000
    ) {
      productsKey = key;
      productsLoadedAt = Date.now();

      productsRequest = request<Product[]>("/store/products")
        .then((items) => {
          if (productsKey === key) {
            productSnapshot = { key, items };
          }
  ...
  recentOrders: () =>
    request<Pick<Order, "id" | "orderNo" | "date" | "status">[]>(
      "/store/orders/recent",
    ),
  ...
  orders: async () => {
    ...
    if (key === (localStorage.getItem("rengas-token") || "")) {
      orderSnapshot = { key, items };
    }
